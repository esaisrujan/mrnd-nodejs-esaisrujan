...
describe("JSIntro Test Suite", function(){
	var JSIntro = require("C:/Users/Srujan/Desktop/nodejs/mrnd-nodejs-master/src/JSIntro");
	...
	describe("JSIntro-ReverseArrayOfStrings",function(){
		it("[abc,mno] reversed is [cba,onm]",function()
		{
			var arrstr=JSIntro.ReverseArrayOfStrings(["abc","mno"]);
			expect(arrstr).toEqual(["cba","onm"]);
		});
		it("[abcde,mnopq,abcdef] reversed is [edcba,qponm,fedcba]",function()
		{
			var arrstr=JSIntro.ReverseArrayOfStrings(["abcde","mnopq","abcdef"]);
			expect(arrstr).toEqual(["edcba","qponm","fedcba"]);
		});
		it("[a] reversed is [a]",function()
		{
			var arrstr=JSIntro.ReverseArrayOfStrings(["a"]);
			expect(arrstr).toEqual(["a"]);
		});
	});
